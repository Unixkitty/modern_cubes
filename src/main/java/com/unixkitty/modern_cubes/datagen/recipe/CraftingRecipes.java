...
//        //LABORATORY_TILE_PLAIN
//        ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, ModBlocks.LABORATORY_TILE_PLAIN.get().asItem(), 32)
...
//
//        //TYRIAN_BLACK
//        ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, ModBlocks.TYRIAN_BLACK.get().asItem(), 32)
//                .define('C', Items.BLACK_CONCRETE)
//                .define('I', Tags.Items.INGOTS_IRON)
//                .pattern("CIC")
//                .pattern("ICI")
//                .pattern("CIC")
//                .unlockedBy("has_item", has(Items.BLACK_CONCRETE))
//                .save(consumer);
    }
    ...
    private void buildStonecutterRecipes(@NotNull Consumer<FinishedRecipe> consumer)
    {
        ...
        //Category stuff
//        ModBlocks.categoryHelpMap.forEach((blockRegistryObject, categoryBlockSupplier) ->
//        {
//            simpleUndoable(blockRegistryObject.get(), categoryBlockSupplier.get(), consumer);
//        });

        Object2ObjectOpenHashMap<Block, ArrayList<Block>> categoryLists = new Object2ObjectOpenHashMap<>();

        ModBlocks.categoryHelpMap.forEach((blockRegistryObject, blockSupplier) ->
        {
            Block mainBlock = blockSupplier.get();
            Block nonMainBlock = blockRegistryObject.get();

            categoryLists.computeIfAbsent(mainBlock, key -> new ArrayList<>()).add(nonMainBlock);
        });

        categoryLists.forEach((mainBlock, categoryBlocks) ->
        {
            Pair<ItemLike, Integer> craftPair = switch (mainBlock.asItem().toString().toLowerCase())
            {
                case "laboratory_tile_plain", "tyrian_black", "hex_black", "space_plating" ->
                        Pair.of(Items.IRON_INGOT, 4);
                case "factory_dots", "plating_copper", "sheetmetal_copper", "soft_plate_white" ->
                        Pair.of(Items.COPPER_INGOT, 4);
                case "asphalt_plain" -> Pair.of(Items.GRAY_CONCRETE, 1);
                case "steel_door" -> Pair.of(Items.IRON_DOOR, 1);
                default -> null;
            };

            if (craftPair != null)
            {
                categoryItem(craftPair.left(), craftPair.right(), consumer, mainBlock, categoryBlocks.toArray(new ItemLike[0]));
            }
        });
...
//        );
        ...
        categoryItem(Items.IRON_INGOT, 2, consumer,
                ModBlocks.BEAM.get(),
                ModBlocks.BEAM_BOLTS.get()
        );
...
//    {
//        categoryItem(ingredient, 4, consumer, mainResult, results);
//    }

    private void categoryItem(ItemLike ingredient, int amount, Consumer<FinishedRecipe> consumer, ItemLike mainResult, ItemLike... results)
    {
        simpleAmount(mainResult, amount, ingredient, consumer);
